...
lines, flags=re.IGNORECASE)
    if 'PEKERJAAN' in lines or 'PEKENAAN' in lines or 'PEKERAAN' in lines:
        if 'PEKERJAAN' in lines:
            print('12')
            pekerjaan_start = lines.index('PEKERJAAN') + 9
        elif 'PEKENAAN' in lines:
            print('13')
            pekerjaan_start = lines.index('PEKENAAN') + 8
        elif 'PEKERAAN' in lines:
            print('14')
            pekerjaan_start = lines.index('PEKERAAN') + 8
        
        pekerjaan_end = lines.find('\n', pekerjaan_start)
        pekerjaan = lines[pekerjaan_start:pekerjaan_end].strip()

        translator = str.maketrans("", "", string.punctuation)
        pekerjaan = pekerjaan.translate(translator)
        

        pekerjaan_lines = pekerjaan  # Store the value in a separate variable
        print(pekerjaan)
        ...
        if match_score >= threshold:
            identity.pekerjaan = closest_word
            print(identity.pekerjaan)
        else:
            print('gak ada')
            identity.pekerjaan = ""
